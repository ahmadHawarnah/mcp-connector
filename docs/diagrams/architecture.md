...
sequenceDiagram
  ...
  G->>G: resolve prefix (e.g., ado_list_work_items)
  C->>G: tools/call {name: "ado_list_work_items", args}
  G->>A: forward tools/call (HTTP)
  A->>ADO API: REST request(s)
  A-->>G: JSON result
  G-->>C: JSON result (+ session headers)
```
...
flowchart LR
  ...
  Server --> Gateway
  Gateway --> Client
  Client --> Result[Answer + Evidence List]
```
